			...
			if(sender instanceof Player){
				if(p.hasPermission("earthspawner.admin")){
					...
					if(args[0].equalsIgnoreCase("help")){
						p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&a&l-------&eEarthSpawners&a-------"));
						p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&f&l/es give <pseudo> <mob> <quantit>">"));
						p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&f&l/es list"));
						p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&f&l/es help"));
						p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&a&l-------------------------------"));
					}else if(args[0].equalsIgnoreCase("list")){
						p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&8&fListe des mobs: cow, pig, creeper, zombie, skeleton, spider, slime, iron_golem, rabbit&f"));
					}else if(args[0].equalsIgnoreCase("give")){
						if(args.length == 1 || args.length == 2 || args.length == 3){
							p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&cMauvais usage : /earthspawner give <Joueur> <Mob> <Quantit")>"));
						}
						if(args.length == 4){
							...
							if(target != null){
								...
								case "pig":
									...
									break								case "rabbit":
									silkspawner.getSpawner(target, "rabbit", amount););												
								default:
									break;
								}
							}else{
								p.sendMessage(ChatColor.translateAlternateColorCodes('&', "Jou&ceur hors ligne !"));
							}
						}
					}
				}else{
					p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&cPas d'perms ! :("));
				}
			}
		}		return false;
	}}
