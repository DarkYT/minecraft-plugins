...
import fr.dark.bedwars.utils.NMSUtils;
import fr.dark.bedwars.utils.PacketUtils;
    ...
    */
    private Object createObjectivePacket(int mode, String displayName)
    {
        try
        {
            ...
            if(mode == 0 || mode == 2)
            {
                ...
                {
                    Object integer = null;
                    for(int i = 0; i < NMSUtils.getClass("IScoreboardCriteria$EnumScoreboardHealthDisplay").getEnumConstants().length; i++)
                        if(NMSUtils.getClass("IScoreboardCriteria$EnumScoreboardHealthDisplay").getEnumConstants()[i].toString().equalsIgnoreCase("INTEGER"))
                            integer = NMSUtils.getClass("IScoreboardCriteria$EnumScoreboardHealthDisplay").getEnumConstants()[i];
    ...
    private Object sendScore(String line, int score)
    {
        ...
        try
        {
            ...
            for(int i = 0; i < NMSUtils.getClass("PacketPlayOutScoreboardScore$EnumScoreboardAction").getEnumConstants().length; i++)
                if(NMSUtils.getClass("PacketPlayOutScoreboardScore$EnumScoreboardAction").getEnumConstants()[i].toString().equalsIgnoreCase("CHANGE"))
                    change = NMSUtils.getClass("PacketPlayOutScoreboardScore$EnumScoreboardAction").getEnumConstants()[i];
    ...
    */
    public class VirtualTeam
    {
        ...
        public void setValue(String value)
        {
            ...
            {
                throw new IllegalArgumentException("Too long value ! Max 48 characters, value was " + value.length() + " (" + value + ") !");
            }
