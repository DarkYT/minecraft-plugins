...
public class StickConfig {
	...
	public String sbWorldName;
	public String sbNetherName;
	
	public String wrongWorld;
	...
	public void loadValues() {
		...
		this.sbWorldName = config.getString("SkyblockWorld");
		this.sbNetherName = config.getString("SkyblockNetherWorld");

		this.wrongWorld = config.getString("WrongWorldMessage").replace("&", "§").replace(":e:", "ê");
	...
	public void setup(File dir) {
		...
		if (!this.conf.exists()) {
			...
			config.set("SkyblockWorld", "world_iles");
			config.set("SkyblockNetherWorld", "world_iles_nether");

			config.set("WrongWorldMessage", "&cVous n'êtes pas dans le bon monde pour utiliser le sellstick.");
